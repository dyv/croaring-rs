...
impl<'a: 'b, 'b> BatchedBitmapIterator<'a> {
    ...
    pub fn next(&'b mut self) -> Option<(u32, &'b [usize; 32])> {
        if self.done {
            return None;
        }
        let n = unsafe {
            ffi::roaring_read_uint64_iterator(self.iterator, self.buffer.as_mut_ptr(), 32)
        };
        if n == 0 {
            self.done = true;
            None
            ...
            Some((n, unsafe { std::mem::transmute(&self.buffer) }))
        }
    }
}
